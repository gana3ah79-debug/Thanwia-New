...
const questionSchema = {
  ...
  properties: {
    ...
    unit: { type: 'string' }, lesson: { type: 'string' }, concept: { type: 'string' },
    skill: { type: 'string' }, source_basis: { type: 'string' },
  },
...
async function geminiJson(prompt: string, schema: any, geminiKey: string) {
...
async function main(req: Request) {
  ...
  const instructions = `أنت محرك أسئلة احترافي للصف الثالث الثانوي المصري. أنشئ أسئلة MCQ أصلية مبنية على المنهج والمراجع المتاحة فقط.\n` +
    `قواعد إلزامية: لا تخترع معلومة أو قاعدة. لا تستخدم محتوى خارج المنهج. لكل سؤال إجابة صحيحة واحدة فقط. كل مشتت يجب أن يكون منطقيًا ومبنيًا على خطأ شائع.\n` +
    `في النحو والبلاغة والأدب: راجع القاعدة أو الخاصية المقصودة بدقة، ولا تخلط بين المحسنات اللفظية والمعنوية، ولا بين المدارس الأدبية وخصائصها، ولا بين شروط الإعمال.\n` +
    `لا تضع نصًا شعريًا أو نثريًا من الذاكرة على أنه اقتباس من كتاب. إذا احتاج السؤال نصًا أصليًا غير متاح في المراجع، صغ مثالًا جديدًا واذكر أنه مثال تطبيقي.\n` +
    `اكتب عربية سليمة، بلا أخطاء إملائية أو نحوية. اجعل السؤال واضحًا ومناسبًا لنمط الثانوية العامة المصرية. نوّع مستويات التفكير والصعوبة. أعد JSON فقط.`;

  const userInput = { request: { subject, grade: 'الصف الثالث الثانوي', branch, unit, lesson, difficulty, count }, reference_sources: sourceContext, already_generated_to_avoid: avoid };
  ...
  let parsed: any;
  try {
    parsed = await geminiJson(`${instructions}\n\nبيانات الطلب والمراجع:\n${JSON.stringify(userInput, null, 2)}`, generationSchema, geminiKey);
  } catch (e) {
  ...
  const candidates = clean.filter((q: any) => { const fp = fingerprint(q); if (seen.has(fp)) return false; seen.add(fp); return true; }).slice(0, count);

  const approved: any[] = [];
  const rejected: any[] = [];
  for (const q of candidates) {
    const reviewPrompt = `أنت مراجع جودة صارم لأسئلة الثانوية العامة المصرية. راجع السؤال التالي مقارنة بالمراجع المرفقة. لا تفترض صحة كلام السؤال لمجرد أنه يبدو مقنعًا.\n` +
      `ارفض السؤال إذا كان فيه: إجابة صحيحة ثانية، قاعدة نحوية/بلاغية غير دقيقة، نسبة خاطئة لمدرسة أدبية، اقتباس غير موثوق، خطأ لغوي جوهري، شرح لا يطابق الإجابة، أو محتوى خارج المنهج.\n` +
      `اقبل فقط إذا كانت الإجابة المحددة صحيحة بوضوح ولا يوجد لبس حقيقي بين الاختيارات. score يجب أن يعكس الجودة، والقبول يتطلب score >= 90.\n` +
      `المراجع:\n${JSON.stringify(sourceContext, null, 2)}\n\nالسؤال:\n${JSON.stringify(q, null, 2)}\n\nأعد JSON فقط.`;
    try {
      const review = await geminiJson(reviewPrompt, reviewSchema, geminiKey);
      if (review?.approved === true && Number(review.score) >= 90) approved.push({ ...q, quality_score: Number(review.score) });
      else rejected.push({ stem: q.stem, issues: review?.issues || ['رفض المراجع'] });
    } catch (e) {
      rejected.push({ stem: q.stem, issues: ['تعذر إتمام مراجعة الجودة'] });
    }
  }

  for (const q of approved) {
    await sb.from('ai_generated_questions').insert({ subject, grade: 'الصف الثالث الثانوي', branch, unit: q.unit || unit, lesson: q.lesson || lesson, concept: q.concept, skill: q.skill, question_type: 'mcq', difficulty: q.difficulty, cognitive_level: q.cognitive_level, stem: q.stem, options: q.options, correct_option: q.correct_option, explanation: q.explanation, common_mistake: q.common_mistake, estimated_seconds: q.estimated_seconds, fingerprint: fingerprint(q), quality_score: q.quality_score, status: 'approved', model: MODEL, generation_run_id: runId });
  }

  if (runId) await sb.from('ai_generation_runs').update({ status: 'completed', count_generated: approved.length }).eq('id', runId);
  return jsonResponse({ questions: approved, model: MODEL, generated: approved.length, candidates: candidates.length, rejected: rejected.length, quality_gate: 'passed-only' });
}
